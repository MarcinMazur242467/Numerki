...
class GUI:
    def __init__(self):
        ...
        self.label = tk.Label(self.root, text="Wprowadź liczbę węzłów czybyszewa: ", font=('Arial', 18))
        self.label.pack(pady=10)

        self.text2 = tk.Text(self.root, height=1, font=('Arial', 12))
        self.text2.pack()

        self.button3 = tk.Button(self.root, text="Wprowadź wezly", command=self.getN)
        self.button3.pack()
    ...
    def gaussChebyshevMetod(self, nodesAmount, epsilon):
        ...
        for i in range(1, nodesAmount + 1):
            ...
            if abs(weight * result - weight * prevRes) <= epsilon:
    ...
    def gaussChebyshevCalculate(self):
        ...
        for i in range(2, Nodes + 1):
            self.text5.insert(tk.END,
                              "Liczba węzłow: " + str(i) + "  -  " + str(self.gaussChebyshevMetod(i, epsilon)) + "\n")
        self.text5.config(state="disabled")
